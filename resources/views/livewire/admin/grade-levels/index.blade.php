<div>
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Grade Levels</h1>
        ...
        <form wire:submit="create" class="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700 mb-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" wire:model="name" placeholder="Secondary 7" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($gradeLevels as $gradeLevel)
                    <tr wire:key="grade-{{ $gradeLevel->id }}">
                        <td class="px-4 py-2 text-gray-500 dark:text-gray-400">{{ $gradeLevel->sort_order }}</td>
                        <td class="px-4 py-2 font-medium">{{ $gradeLevel->name }}</td>
                        <td class="px-4 py-2 text-gray-500 dark:text-gray-400">{{ $gradeLevel->classes_count }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="3" class="px-4 py-6 text-center text-gray-500 dark:text-gray-400">No grade levels yet.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
