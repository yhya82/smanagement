<div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <select wire:model.live="roleId" class="rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($users as $user)
                    <tr wire:key="user-{{ $user->id }}">
                        ...
                        <td class="px-4 py-2 text-gray-500 dark:text-gray-400">{{ $user->roles->pluck('name')->join(', ') ?: '-' }}</td>
                        <td class="px-4 py-2">
                            <span @class([
                        ...
                        <td class="px-4 py-2 text-right">
                            <a href="{{ route('admin.users.show', $user) }}" wire:navigate class="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-500">Manage</a>
                        </td>
                    </tr>
                @empty
                    <tr>
