<div class="max-w-2xl">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">{{ $teacher->user->name }}</h1>
            <p class="text-sm text-gray-500">{{ $teacher->employee_no }} - {{ $teacher->user->email }}</p>
        </div>
        <a href="{{ route('admin.teachers.index') }}" wire:navigate class="text-sm text-indigo-600 hover:text-indigo-500">Back to teachers</a>
    </div>

    <form wire:submit="assign" class="bg-white p-6 rounded-lg border border-gray-200 mb-6 space-y-4">
        <h2 class="text-sm font-medium text-gray-900">Assign to a subject/class</h2>
