        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left font-medium text-gray-500">Permissions</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($roles as $role)
                    <tr wire:key="role-{{ $role->id }}">
                        <td class="px-4 py-2 font-medium">
                            {{ $role->name }}
                            @if ($role->is_system)
                                <span class="ml-1 inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">System</span>
                            @endif
                        </td>
                        <td class="px-4 py-2 text-gray-500">{{ $role->description ?: '-' }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $role->permissions_count }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $role->users_count }}</td>
                        <td class="px-4 py-2">
                            <span @class([
                                'inline-flex px-2 py-0.5 rounded-full text-xs font-medium',
                                'bg-green-100 text-green-800' => $role->is_active,
                                'bg-gray-100 text-gray-600' => ! $role->is_active,
                            ])>
                                {{ $role->is_active ? 'Active' : 'Disabled' }}
                            </span>
                        </td>
                        <td class="px-4 py-2 text-right whitespace-nowrap">
                            <a href="{{ route('admin.roles.show', $role) }}" wire:navigate class="text-xs text-indigo-600 hover:text-indigo-500">Manage</a>
                            @unless ($role->is_system)
                                ...
                                <button type="button" wire:click="toggleActive({{ $role->id }})"
                                    ...
                                    class="text-xs {{ $role->is_active ? 'text-red-600 hover:text-red-500' : 'text-green-600 hover:text-green-500' }}">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-6 text-center text-gray-500">No roles found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
