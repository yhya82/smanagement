<div>
    <h1 class="text-xl font-semibold text-gray-900 mb-6">Students</h1>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($students as $student)
                    <tr wire:key="student-{{ $student->id }}">
                        <td class="px-4 py-2 text-gray-500">{{ $student->student_no }}</td>
                        <td class="px-4 py-2 font-medium">{{ $student->first_name }} {{ $student->last_name }}</td>
                        <td class="px-4 py-2 text-gray-500">{{ $student->currentClass?->name ?? '-' }}</td>
                        <td class="px-4 py-2">
                            <span @class([
                        ...
                        <td class="px-4 py-2 text-right">
                            <a href="{{ route('admin.students.show', $student) }}" wire:navigate class="text-xs text-indigo-600 hover:text-indigo-500">View</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-6 text-center text-gray-500">No students found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
